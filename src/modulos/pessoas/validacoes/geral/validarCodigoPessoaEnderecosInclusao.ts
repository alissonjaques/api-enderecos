...
function validarCodigoPessoaEnderecosInclusao(
  ...
  for (let i = 0; i < enderecos.length; i++) {
    ...
    if (endereco.codigoPessoa && endereco.codigoPessoa !== codigoPessoa) {
      throw new AppErros(
        `Não foi possível inserir o endereço no banco de dados. Motivo: o campo codigoPessoa = ${codigoPessoa} da pessoa é diferente do campo codigoPessoa = ${endereco.codigoPessoa} do endereço`,
      );
    }
  }
}
